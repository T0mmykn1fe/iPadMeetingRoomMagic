...
function wrapGEvent(gevent, fallbackTimezone, ds) {
    ...
    var organizer = gevent.organizer &&
        (
            gevent.organizer.displayName ||
            (emailRegex.test(gevent.organizer.email || '') && gevent.organizer.email.match(emailRegex)[1])
        );
    return new Event(
        isPrivate ? "Private event" : gevent.summary,
        isPrivate ? "Hidden organizer" : organizer || 'None',
